...
const DC_API = () =>
  vscode.workspace.getConfiguration('danteclicky').get<string>('apiBase', 'http://127.0.0.1:9002');

async function dcFetch(path: string, init?: RequestInit): Promise<unknown> {
  const res = await fetch(`${DC_API()}${path}`, {
    ...init,
    headers: { 'Content-Type': 'application/json', ...(init?.headers ?? {}) },
  });
  if (!res.ok) throw new Error(`DanteClicky HTTP ${res.status} at ${path}`);
  return res.json();
}

async function checkDcRunning(): Promise<boolean> {
  try { await dcFetch('/health'); return true; } catch { return false; }
}

async function askDanteClicky(prompt: string): Promise<string> {
...
class DanteClickySidebarProvider implements vscode.WebviewViewProvider {
  private _view?: vscode.WebviewView;
  private _dcRunning = false;
  ...
  resolveWebviewView(
    ...
    webviewView.webview.onDidReceiveMessage(async (msg: { type: string; prompt?: string }) => {
      switch (msg.type) {
        case 'refresh':
          await this._refresh();
          break;
        case 'ask':
          if (msg.prompt) await this._handleAsk(msg.prompt);
          break;
        case 'screenshot':
  ...
  private async _refresh() {
    ...
    try {
      const health = await dcFetch('/health') as { ok: boolean; sessions: number };
      const tools = await dcFetch('/v1/tools') as { tools: Array<{ name: string; description: string }> };
  ...
  private async _handleAsk(prompt: string) {
    const view = this._view;
    if (!view) return;
  ...
  private async _handleScreenshot() {
    ...
    if (!view) return;
    try {
      const result = await dcFetch('/v1/screenshot?monitor=0') as { data?: string; width: number; height: number };
...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider('danteclicky.sidebar', sidebarProvider)
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('danteclicky.ask', async () => {
      ...
      await vscode.window.withProgress(
        { location: vscode.ProgressLocation.Notification, title: 'Asking DanteClicky…', cancellable: false },
        async () => {
          try {
  ...
  // Command: Screenshot
  context.subscriptions.push(
    vscode.commands.registerCommand('danteclicky.screenshot', async () => {
      try {
        const result = await dcFetch('/v1/screenshot?monitor=0') as { width: number; height: number };
  ...
  // Command: Status
  context.subscriptions.push(
    vscode.commands.registerCommand('danteclicky.status', async () => {
      try {
        const health = await dcFetch('/health') as { ok: boolean; sessions: number };
        const tools = await dcFetch('/v1/tools') as { tools: unknown[] };
        vscode.window.showInformationMessage(
          `DanteClicky: ${health.ok ? 'Running' : 'Error'} | ${tools.tools.length} tools | ${health.sessions} SSE sessions`
        );
      } catch {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('danteclicky.sendWebhook', async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) return;
