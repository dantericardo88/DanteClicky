...
mod tests {
    ...
    fn test_create_session_and_get_history() {
        ...
        // Oldest-first order: user message comes before assistant.
    ...
    fn test_get_history_limit() {
        ...
        // Limit 1 should return only the most recent message.
    ...
    fn test_fts5_search_finds_by_keyword() {
        ...
        // Search for "quantum" — should find the two quantum messages.
        let results = db.search("quantum", 20).unwrap();
        assert_eq!(results.len(), 2, "search should find both quantum messages");

        // Verify the neural-network message is NOT in results.
        for r in &results {
            assert!(
                r["content"]
                    .as_str()
